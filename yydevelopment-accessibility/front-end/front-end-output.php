    ...
    if( $accessibility_plugin_data_array['display_button_checkbox'] == 1 ) {
        ...
        add_action( 'wp_head', function() use ($accessibility_plugin_data_array, $accessibility_exclude_option, $accessibility_exclude_ids_array) {
           ...
           if( $output_button_now ) {
                ...
                $icon_type = isset($accessibility_plugin_data_array['icon_type']) ? esc_html($accessibility_plugin_data_array['icon_type']) : '';
                $icon_svg_url = isset($accessibility_plugin_data_array['icon_svg_url']) ? esc_url($accessibility_plugin_data_array['icon_svg_url']) : '';
                $icon_width = isset($accessibility_plugin_data_array['icon_width']) ? intval($accessibility_plugin_data_array['icon_width']) : 0;
                $icon_height = isset($accessibility_plugin_data_array['icon_height']) ? intval($accessibility_plugin_data_array['icon_height']) : 0;
                ...
                $z_index = intval($accessibility_plugin_data_array['z_index']);
                        ...
                        $yydev_accessibility_style .= '<style>';
                            ...
                            $yydev_accessibility_style .= '.yydev-accessibility .yy-button {';
                                    ...
                                    if ($icon_type === 'svg') {
                                        // For SVG icons, use background image like regular images
                                        $current_svg_url = !empty($icon_svg_url) ? $icon_svg_url : plugins_url('images/accessibility-icon.svg', dirname(__FILE__));
                                        $yydev_accessibility_style .= 'background:' . $background_color . ' url(' . $current_svg_url .') no-repeat center center;';
                                        
                                        // Use specific SVG dimensions if set, otherwise use contain
                                        if (!empty($icon_width) && $icon_width > 0 && !empty($icon_height) && $icon_height > 0) {
                                            $yydev_accessibility_style .= 'background-size: ' . intval($icon_width) . 'px ' . intval($icon_height) . 'px;';
                                        } else {
                                            $yydev_accessibility_style .= 'background-size: contain;';
                                        }
                                        
                                        $yydev_accessibility_style .= 'text-indent:-9999px;';
                                    } else {
                                        // Default behavior for images
                                        $current_icon_url = !empty($icon_image_url) ? $icon_image_url : plugins_url('images/accessibility-icon.png', dirname(__FILE__));
                                        $yydev_accessibility_style .= 'background:' . $background_color . ' url(' . $current_icon_url .') no-repeat 45% 50%;';
                                        $yydev_accessibility_style .= 'text-indent:-9999px;';
                                    }
                ...
                if( $output_button_now ) {
                        ...
                        $yydev_accessibility_html .= "<div class='yydev-accessibility no-accessibility " . $direction_code . "'>";
                            $yydev_accessibility_html .= "<div class='yydev-warp'>";
                                ...
                                // Generate button HTML - same for both image and SVG types since both use background images now
